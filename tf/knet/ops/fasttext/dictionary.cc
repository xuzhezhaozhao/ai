...
const std::string& Dictionary::getWord(int32_t id) const {
  assert(id >= 0);
  assert(id < size_);
  return words_[id].word;
}
int64_t Dictionary::getWordCount(int32_t id) const {
  assert(id >= 0);
  assert(id < size_);
...
void Dictionary::readFromFile(std::istream& in) {
  ...
  if (size_ == 0) {
    LOG(INFO) << "Empty vocabulary. Try a smaller min_count value.";
    exit(EXIT_FAILURE);
  }
}
...
void Dictionary::initTableDiscard() {
  ...
  for (size_t i = 0; i < std::min(200, size_); i++) {
    LOG(INFO) << "pdiscard " << i << " = " << pdiscard_[i];
  }
}
...
int32_t Dictionary::getLine(std::istream& in,
  ...
  addWordNgrams(words, word_hashes, args_->word_ngrams);
  return ntokens;
}
