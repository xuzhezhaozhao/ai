...
flags.DEFINE_integer("save_summary_steps", 10,
                     "How often to save the summaries.")
...
class XnliProcessor(DataProcessor):
    ...
    def __init__(self, data_dir):
        super(XnliProcessor, self).__init__(data_dir)
        self.language = "zh"

    def get_train_examples(self):
        """See base class."""
        lines = self._read_tsv(
            os.path.join(self.data_dir, "multinli",
    ...
    def get_dev_examples(self):
        """See base class."""
        lines = self._read_tsv(os.path.join(self.data_dir, "xnli.dev.tsv"))
        examples = []
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
...
class MnliProcessor(DataProcessor):
    ...
    def __init__(self, data_dir):
        super(MnliProcessor, self).__init__(data_dir)
    ...
    def get_test_examples(self):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(self.data_dir, "test_matched.tsv")),
            "test")
...
class MrpcProcessor(DataProcessor):
    ...
    def __init__(self, data_dir):
        super(MrpcProcessor, self).__init__(data_dir)
...
class ColaProcessor(DataProcessor):
    ...
    def __init__(self, data_dir):
        super(ColaProcessor, self).__init__(data_dir)
...
class FasttextProcessor(DataProcessor):
    """Processor for the data whoes format is similar with fasttext."""

    def __init__(self, data_dir):
        super(FasttextProcessor, self).__init__(data_dir)
    ...
    def get_labels(self):
        """See base class."""
        labels = []
        for line in open(os.path.join(self.data_dir, "label.txt")):
            label = line.strip()
            if label != "":
                labels.append(label)
        return labels
...
def main(_):
    ...
    if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict:
        raise ValueError(
            "At least one of `do_train`, `do_eval` or `do_predict' must be "
            "True.")
    ...
    config_keys = {}
    config_keys['model_dir'] = FLAGS.output_dir
    ...
    estimator_keys = {}
    estimator_keys['model_fn'] = model_fn
    estimator_keys['config'] = run_config
