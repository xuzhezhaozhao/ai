...
int sgeqrf_(FINTEGER *m, FINTEGER *n, float *a, FINTEGER *lda, float *tau,
            float *work, FINTEGER *lwork, FINTEGER *info);

int sorgqr_(FINTEGER *m, FINTEGER *n, FINTEGER *k, float *a, FINTEGER *lda,
...
RandomGenerator::RandomGenerator(long seed) : mt((unsigned int)seed) {}
...
#pragma omp parallel for
  for (size_t j = 0; j < nblock; j++) {
    ...
    for (size_t i = istart; i < iend; i++) x[i] = rng.rand_float();
  }
}
...
#pragma omp parallel for
  for (size_t j = 0; j < nblock; j++) {
    ...
    const size_t istart = j * n / nblock;
    const size_t iend = (j + 1) * n / nblock;
    for (size_t i = istart; i < iend; i++) x[i] = rng.rand_long();
  }
}
...
#pragma omp parallel for
  for (size_t j = 0; j < nblock; j++) {
    ...
    size_t i;
    for (i = istart; i < iend; i++) x[i] = rng.rand_long();
  }
}

void reflection(const float *__restrict u, float *__restrict x, size_t n,
                size_t d, size_t nu) {
  size_t i, j, l;
  for (i = 0; i < n; i++) {
    const float *up = u;
...
void fvec_inner_products_ny(float *ip, const float *x, const float *y, size_t d,
                            size_t ny) {
...
void fvec_norms_L2(float *__restrict nr, const float *__restrict x, size_t d,
                   size_t nx) {
#pragma omp parallel for
...
void fvec_norms_L2sqr(float *__restrict nr, const float *__restrict x, size_t d,
                      size_t nx) {
#pragma omp parallel for
  for (size_t i = 0; i < nx; i++) nr[i] = fvec_norm_L2sqr(x + i * d, d);
}
...
static void knn_inner_product_sse(const float *x, const float *y, size_t d,
                                  size_t nx, size_t ny,
...
static void knn_L2sqr_sse(const float *x, const float *y, size_t d, size_t nx,
...
/** Find the nearest neighbors for nx queries in a set of ny vectors */
static void knn_inner_product_blas(const float *x, const float *y, size_t d,
                                   size_t nx, size_t ny,
                                   float_minheap_array_t *res) {
  res->heapify();
    ...
    for (size_t j0 = 0; j0 < ny; j0 += bs_y) {
      ...
      /* compute the actual dot products */
      {
        ...
        sgemm_("Transpose", "Not transpose", &nyi, &nxi, &di, &one, y + j0 * d,
               &di, x + i0 * d, &di, &zero, ip_block, &nyi);
      }
...
static void knn_L2sqr_blas(const float *x, const float *y, size_t d, size_t nx,
                           size_t ny, float_maxheap_array_t *res,
...
static void range_search_blas(const float *x, const float *y, size_t d,
  ...
  for (size_t j0 = 0; j0 < ny; j0 += bs_y) {
    ...
    for (size_t i0 = 0; i0 < nx; i0 += bs_x) {
      ...
      for (size_t i = i0; i < i1; i++) {
        ...
        RangeSearchPartialResult::QueryResult &qres = pres->new_result(i);
...
template <bool compute_l2>
static void range_search_sse(const float *x, const float *y, size_t d,
                             size_t nx, size_t ny, float radius,
...
void range_search_L2sqr(const float *x, const float *y, size_t d, size_t nx,
                        size_t ny, float radius, RangeSearchResult *res) {
  if (d % 4 == 0 && nx < distance_compute_blas_threshold) {
...
#pragma omp parallel for
  for (size_t j = 0; j < n1; j++) {
    float *disj = dis + j * n2;
    for (size_t i = 0; i < n2; i++) disj[i] = nr1[j] + nr2[i] - 2 * disj[i];
  }
}
...
void pairwise_L2sqr(long d, long nq, const float *xq, long nb, const float *xb,
                    float *dis, long ldq, long ldb, long ldd) {
  if (nq == 0 || nb == 0) return;
  if (ldq == -1) ldq = d;
  if (ldb == -1) ldb = d;
  if (ldd == -1) ldd = nb;
...
#pragma omp parallel for
  for (long i = 0; i < nb; i++) b_norms[i] = fvec_norm_L2sqr(xb + i * ldb, d);

#pragma omp parallel for
  for (long i = 1; i < nq; i++) {
    float q_norm = fvec_norm_L2sqr(xq + i * ldq, d);
    for (long j = 0; j < nb; j++) dis[i * ldd + j] = q_norm + b_norms[j];
  }

  {
    float q_norm = fvec_norm_L2sqr(xq, d);
    for (long j = 0; j < nb; j++) dis[j] += q_norm;
  }

  {
    ...
    sgemm_("Transposed", "Not transposed", &nbi, &nqi, &di, &minus_2, xb, &ldbi,
           xq, &ldqi, &one, dis, &lddi);
  }
}
...
size_t merge_result_table_with(size_t n, size_t k, long *I0, float *D0,
                               const long *I1, const float *D1, bool keep_min,
                               long translation) {
  size_t n1 = 0;
...
void bincode_hist(size_t n, size_t nbits, const uint8_t *codes, int *hist) {
  ...
  for (size_t i = 0; i < n; i++)
    for (int j = 0; j < d; j++) accu[j * 256 + *c++]++;
...
template <typename T>
void parallel_merge(const T *src, T *dst, SegmentS &s1, SegmentS &s2, int nt,
...
#pragma omp parallel for num_threads(nseg1)
    for (int s = 0; s < nseg; s += 2) {
      if (s + 1 == nseg) {  // otherwise isolated segment
        ...
        parallel_merge(permA, permB, segs[s], segs[s + 1], t1 - t0, comp);
      }
    }
    for (int s = 0; s < nseg; s += 2) segs[s / 2] = segs[s];
    nseg = nseg1;
    std::swap(permA, permB);
  }
  assert(permA == perm);
...
const float *fvecs_maybe_subsample(size_t d, size_t *n, size_t nmax,
                                   const float *x, bool verbose, long seed) {
