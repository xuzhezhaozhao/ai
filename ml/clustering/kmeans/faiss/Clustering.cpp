...
void Clustering::train(idx_t nx, const float *x_in, Index &index) {
  FAISS_THROW_IF_NOT_FMT(nx >= k,
                         ...
                         "as large as number of clusters (%ld)",
                         nx, k);
  ...
  for (int redo = 0; redo < nredo; redo++) {
    ...
    for (int i = n_input_centroids; i < k; i++)
      memcpy(&centroids[i * d], x + perm[i] * d, d * sizeof(float));
    ...
    for (int i = 0; i < niter; i++) {
      ...
      err = 0;
      for (int j = 0; j < nx; j++) err += dis[j];
      obj.push_back(err);

      int nsplit =
          km_update_centroids(x, centroids.data(), assign, d, k, nx,
                              frozen_centroids ? n_input_centroids : 0);

      if (verbose) {
        printf(
            "  Iteration %d (%.2f s, search %.2f s): "
            "objective=%g imbalance=%.3f nsplit=%d       \r",
            i, (getmillisecs() - t0) / 1000.0, t_search_tot / 1000, err,
            imbalance_factor(nx, k, assign), nsplit);
        fflush(stdout);
      }

      if (spherical) fvec_renorm_L2(d, k, centroids.data());

      index.reset();
      if (update_index) index.train(k, centroids.data());
    ...
    if (nredo > 1) {
      if (err < best_err) {
        if (verbose) printf("Objective improved: keep new clusters\n");
