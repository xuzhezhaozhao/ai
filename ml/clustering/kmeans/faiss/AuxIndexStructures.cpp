...
void BufferList::copy_range(size_t ofs, size_t n, idx_t *dest_ids,
                            float *dest_dis) {
...
/// begin a new result
RangeSearchPartialResult::QueryResult &RangeSearchPartialResult::new_result(
    idx_t qno) {
...
void RangeSearchPartialResult::set_result(bool incremental) {
  ...
  for (int i = 0; i < queries.size(); i++) {
    ...
    copy_range(ofs, qres.nres, res->labels + res->lims[qres.qno],
