...
# see https://github.com/keras-team/keras-contrib/issues/280
#
# The paper calls this second network not a discriminator but a critic.
# The important difference is, that the network does not try to match the
# labels y_true, but tries to minimize the loss K.mean(y_true * y_pred).
# Essential the critic tries to minimize the output y_pred for real images
# (this is +1 * y_pred) and maximize the output for fake images (as it tries
# to minimize -1 * y_pred). If you change the sign in this definition,
# the critic simply has to learn a sign flip compared to the previous setting,
# which should be not a problem. Also if the critic can't distinguish, we have
# that the loss is indeed 0, as the expectation value of y_pred is independent
# of y_true, and as half of your data is real (y_true=1) and half is fake
# (y_true=-1) the loss will be 0. If the critic is able to distinguish, it
# will have a loss smaller than 0, if it minimizes.
def build(x):
    # (1) Update D network
    # train with real
    ...
    with tf.name_scope('clip_op'):
        clip_op = [v.assign(tf.clip_by_value(v, -0.01, 0.01)) for v in d_vars]
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops):
        with tf.control_dependencies(clip_op):
