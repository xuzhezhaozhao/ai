...
void FastText::precomputeWordVectors() {
  if (!precomputed_) {
...
void FastText::findNN(const Matrix& wordVectors, const Vector& queryVec,
  ...
  for (int32_t i = 0; i < dict_->nwords(); i++) {
    std::string word = dict_->getWord(i);
    size_t count = dict_->getWordCount(i);
    if (count <= (size_t)t) {
      continue;
    }
...
void FastText::findNN(const Matrix& wordVectors, const Vector& queryVec,
  ...
  while (i < k && heap.size() > 0) {
    auto it = banSet.find(heap.top().second);
    if (it == banSet.end()) {
      nn.push_back(std::make_pair(heap.top().first, heap.top().second));
      i++;
    }
    heap.pop();
  }
...
void FastText::nn(int32_t k, int32_t t) {
  ...
  while (std::cin >> queryWord) {
    ...
    findNN(wordVectors, queryVec, k, t, banSet);
  }
}
...
void FastText::analogies(int32_t k) {
  ...
  while (true) {
    ...
    findNN(wordVectors, query, k, 0, banSet);
    std::cout << "Query triplet (A - B + C)? ";
  }
}
